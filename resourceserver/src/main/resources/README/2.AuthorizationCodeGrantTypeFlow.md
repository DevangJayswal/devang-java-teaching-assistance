...
* CONFIDENTIAL CLIENT: If your web application is using Spring MVC then we can use
 Authorization Code Flow by storing `client_secret` within the web server.

* after AUTHENTICATION, we get authorization `code`
so when we say I have an authorization `code` that means I am authenticated
* after AUTHORIZATION, we get `access_token`
so when we say I have an `access_token` that means I am authenticated and authorized

* In order to AUTHENTICATE yourself, you will have to provide `username` and `password` to authorization server
* In order to AUTHORIZE yourself, you will have to provide authorization `code` to authorization server
* Once you are AUTHORIZED you get an `access_token` to access your protected resources
...
10. client uses `access_token` in order to access his protected resources
