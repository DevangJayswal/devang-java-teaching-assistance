...
ENDPOINTS:
    1. token: to get the `access_token`
    2. authorization: to get the authorization `code`
...
INVOKE -> POST `token_endpoint` -> BODY -> x-www-form-urlencoded
       ...
       * grant_type: client_credentials (when there is no user is involved because in this use case
